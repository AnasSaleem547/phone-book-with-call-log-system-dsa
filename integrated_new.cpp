...
#include <sstream>
#include <string>
#include "integrated_operations.h" // Includes CallLogManager class
using namespace std;
class td;
...
void makeAndLogCall(CallLogManager& callManager, const string& numberToCall); //overloaded declaration
int keycount = 0;
class Group;
...
class td{ // td means telephone directory
public:
    node *head;
	...
	node* searchbyname(string name) {
	    ...
	    cout << "Searching for contacts with the name: " << name << endl;
	    while (temp != NULL) {
	        if (temp->fname.find(name) != string::npos || temp->lname.find(name) != string::npos) {
	            cout << "Contact found: " << endl;
	            cout << "First name: " << temp->fname << endl;
	            cout << "Last name: " << temp->lname << endl;
	            cout << "Numbers: ";
	            for (const string& num : temp->num) {
	                cout << num << " ";
	            }
	            cout << endl << endl;
	            found = true;
	    ...
	    if (!found) {
	        cout << "Contact not found. Would you like to add a new contact? (Y/N): ";
	        char addChoice;
	        cin >> addChoice;
	...
	//overloaded searchbyname
	void searchbyname(string name, CallLogManager& callManager) {
	    name = capitalize(name);
	    node *temp = head;
	    bool found = false;
	    ...
	    while (temp != NULL) {
	        if (temp->fname.find(name) != string::npos || temp->lname.find(name) != string::npos) {
	            cout << "First name: " << temp->fname << ", Last name: " << temp->lname << ", Numbers: ";
	            for (const string& num : temp->num) {
	                cout << num << " ";
	            }
	            cout << endl;
	    ...
	    if (found) {
	        cout << "Do you want to call any of these contacts? (Y/N): ";
	        char callChoice;
	        cin >> callChoice;
	        if (toupper(callChoice) == 'Y') {
	            string numberToCall;
	            cout << "Enter the phone number to call: ";
	            cin >> numberToCall;
	            makeAndLogCall(callManager, numberToCall); // Call the overloaded function
	        }
	    } else {
...
node* searchbynumber(string number) {
	...
	void viewContactsByLetter(char letter) {
	    ...
	    int index = letter - 'A'; // Convert the letter to an index (0-25)
	
	    if (index < 0 || index >= 26 || letterHead[index] == nullptr) {
...
//normal makeandlogcall
void makeAndLogCall(CallLogManager& callManager) {
    cout << "Do you want to call any of these contacts? (Y/N): ";
    char callChoice;
    cin >> callChoice;
    if (toupper(callChoice) == 'Y') {
        ...
        cout << "Enter the phone number to call: ";
        cin >> numberToCall;
        time_t callStart = time(nullptr);
        cout << "Calling " << numberToCall << "..." << endl;
        cout << "Press any key to end the call..." << endl;
        _getch(); // Wait for user input to end the call
        time_t callEnd = time(nullptr);
        double callDuration = difftime(callEnd, callStart);
        callManager.addCallLog(numberToCall, "Outgoing", callStart, callEnd, callDuration);
        cout << "Call ended. Duration: " << callDuration << " seconds." << endl;
    }
    system("pause");
}

//overloaded to be used for OPTION E
void makeAndLogCall(CallLogManager& callManager, const string& numberToCall) {
    time_t callStart = time(nullptr);
    cout << "Calling " << numberToCall << "..." << endl;
    cout << "Press any key to end the call..." << endl;
    _getch(); // Wait for user input to end the call
...
int main() {
    ...
    CallLogManager callManager; // Instantiate call log manager
...
menu:
    ...
    cout << "X. Make a Call" << endl;
    cout << "Y. Add Call Log" << endl;
    cout << "Z. View Call Logs" << endl;
    cout << "G. Exit" << endl;

    cin >> choice;
	
	// As there's no actual call, using current time for start and end
		    time_t now = time(nullptr);
		    double duration = 0;  // Duration is 0 as there's no actual call duration
    switch (choice) {
	    ...
	    case 'E':{
		    system("cls");
		    string searchTerm;
		    cout << "Enter name or number for you want to search:";
		    cin >> searchTerm;
		    searchTerm = capitalize(searchTerm);
		    t.searchbyname(searchTerm, callManager); // Now passing the search term and callManager
		    system("pause");
		    goto menu;
		    break;
		}
        ...
        case 'X': {
            ...
            cin >> number;
            // Simulate a call (for example, just a message for now)
            cout << "Calling " << number << "..." << endl;
            // Add the call log
            callManager.addCallLog(number, "Outgoing", now, now, duration);
            cout << "Call ended. Call log added successfully." << endl;
            system("pause");
            goto menu;
        }
		...
		case 'Y': {
		    system("cls");
		    string number;
		    cout << "Enter phone number for the call log: ";
		    cin >> number;
		
		    
		
		    callManager.addCallLog(number, "Outgoing", now, now, duration);
		    cout << "Call log added successfully." << endl;
		    system("pause");
		    goto menu;
		    break;
		}
